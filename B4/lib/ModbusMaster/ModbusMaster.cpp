...
/* _____PUBLIC FUNCTIONS____________________________________________ */
...
ModbusMaster::ModbusMaster(uint8_t u8SerialPort, uint8_t u8MBSlave, uint8_t u8txenpin)
{
  _portno = u8SerialPort;
  switch( u8SerialPort ) {
    #if defined(UBRR1H)
...
//  txBuffer = (uint16_t*) calloc(ku8MaxBufferSize, sizeof(uint16_t));
   ...
   if (_u8txenpin > 1) { 		// pin 0 & pin 1 are reserved for RX/TX
      pinMode(_u8txenpin, OUTPUT);
      digitalWrite(_u8txenpin, LOW);
    }
}
...
and response from slave. Do not call functions that read from the 
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  if (_u8txenpin > 1)  {
	...
	digitalWrite(_u8txenpin, HIGH );  
  }
  delay(10);

  for (i = 0; i < u8ModbusADUSize; i++)
  {
	port->write(u8ModbusADU[i]);//W232.send(u8ModbusADU[i], BYTE);
  }
 ...
 if (_u8txenpin > 1){
  ...
  {
  if  (port->available())//(W232.available())
  {    
		u8ModbusADU[u8ModbusADUSize] = port->read();//W232.read();
		u8BytesLeft--;
		u8ModbusADUSize ++;
