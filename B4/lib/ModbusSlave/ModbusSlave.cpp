    ...
    default:
      ...
      break;
        ...
        for (i = 0; i < string_length; i++) {
                port->write(query[i]); //W232.send(query[i], BYTE);
        }

	// Set Txpin ----> for receive
...
int ModbusSlave::receive_request(unsigned char *received_string) 
{
        ...
        while (port->available()) {
                received_string[bytes_received] = port->read();//W232.read();
                bytes_received++;
...
void ModbusSlave::configure(unsigned char SerialPort, unsigned char slave, long baud, char txpin)
{
  portno = SerialPort;
  switch( SerialPort ) {
    #if defined(UBRR1H)
  ...
  if (txpin > 1) { // pin 0 & pin 1 are reserved for RX/TX
